            ...
            stSQL = "SELECT rid,idk,nama,bank,norek as no_rek,tanggal,atasnama as atas_nama,aktif,keterangan,created_at,updated_at " &
                    "FROM ( " &
                        ...
                        "FROM ( " &
                            ...
                            "WHERE ((upper(tbl." & mSelectedCriteria & ") LIKE '%" & mKriteria.ToUpper & "%')) AND EXISTS(select 1 from " & CONN_.schemaHRD & ".mskaryawanaktif mk where tbl2.idk=mk.idk and mk.perusahaan like '%" & USER_.entityChose & "') " & IIf(USER_.lokasi = "ALL", "", "AND (tbl2.lokasi='" & myCStringManipulation.SafeSqlLiteral(USER_.lokasi) & "')") & " " &
