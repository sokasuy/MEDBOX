...
Public Class FormProsesPayroll
    ...
    Private Sub FormProsesPayroll_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Try
            ...
            Else
                dtpPeriodeMulai.Value = DateSerial(Now.Year, Now.Month, 21)
        ...
        Try
            ...
            ElseIf (_kelompok = "STAFF") Then
                ...
                ElseIf (_departemen = "APOTEK") Then
                    ...
                    stSQL = "SELECT count(p.nip) as totaltidakmasuk,p.absen,k.keterangan,k.potonggaji,k.persengaji,k.potongtunjangankehadiran,k.potongtunjangantransport 
                            ...
                            GROUP BY p.absen,k.keterangan,k.potonggaji,k.persengaji,k.potongtunjangankehadiran,k.potongtunjangantransport
                            ORDER BY p.absen;"
            ...
            'TERLAMBAT
            terlambat = myCDBOperation.GetFormulationRecord(CONN_.dbMain, CONN_.comm, CONN_.reader, "nip", CONN_.schemaHRD & ".trdatapresensi", "Count", "nip='" & myCStringManipulation.SafeSqlLiteral(_nip) & "' AND (tanggal>='" & Format(_periodeMulai, "dd-MMM-yyyy") & "' AND tanggal<='" & Format(_periodeSelesai, "dd-MMM-yyyy") & "') AND terlambat is not null", CONN_.dbType)
            If (terlambat > 0) Then
                stSQL = "SELECT d.dendaharian,d.toleransi,d.dendapenalty FROM " & CONN_.schemaHRD & ".msdendaterlambat as d inner join " & CONN_.schemaHRD & ".msposisikaryawan as p on d.leveljabatan=p.level WHERE p.nip='" & myCStringManipulation.SafeSqlLiteral(_nip) & "';"
                ...
                If (myDataDendaTerlambat.Rows.Count > 0) Then
                    If (terlambat > myDataDendaTerlambat.Rows(0).Item("toleransi")) Then
                        nominalTerlambat = (terlambat * myDataDendaTerlambat.Rows(0).Item("dendaharian")) + myDataDendaTerlambat.Rows(0).Item("dendapenalty")
                    Else
            ...
            If (myDataTableAbsen.Rows.Count > 0) Then
                ...
                For i As Integer = 0 To myDataTableAbsen.Rows.Count - 1
                    If (keteranganAbsenLama <> myDataTableAbsen.Rows(i).Item("keterangan")) Then
                        ...
                        potonganTidakMasuk = myDataTableAbsen.Rows(i).Item("totaltidakmasuk") * ((upahPokok * (IIf(myDataTableAbsen.Rows(i).Item("persengaji") = 100, 0, 100) / 100)) / hariKerja1Periode + ((IIf(myDataTableAbsen.Rows(i).Item("potongtunjangankehadiran"), tunjanganHadir, 0) + IIf(myDataTableAbsen.Rows(i).Item("potongtunjangantransport"), tunjanganTransport, 0)) / hariKerja1Periode))
            ...
            'CEK KALENDER
            'stSQL = "SELECT tanggal,katlibur FROM kalenderperusahaan as kp WHERE libur='True' and katlibur='LIBUR NASIONAL' and tanggal>='" & Format(_periodeMulai, "dd-MMM-yyyy") & "' and tanggal<='" & Format(_periodeSelesai, "dd-MMM-yyyy") & "' and lokasi='" & myCStringManipulation.SafeSqlLiteral(_lokasi) & "';"
            stSQL = "SELECT count(kp.tanggal) as banyakmasuk FROM " & CONN_.schemaHRD & ".kalenderperusahaan as kp inner join " & CONN_.schemaHRD & ".trdatapresensi as p on kp.tanggal=p.tanggal and kp.lokasi=p.lokasi WHERE p.nip='" & myCStringManipulation.SafeSqlLiteral(_nip) & "' AND p.absen is null AND p.banyakjamkerjanyata is not null AND kp.libur='True' and kp.katlibur='LIBUR NASIONAL' and p.tanggal>='" & Format(_periodeMulai, "dd-MMM-yyyy") & "' and p.tanggal<='" & Format(_periodeSelesai, "dd-MMM-yyyy") & "' and p.lokasi='" & myCStringManipulation.SafeSqlLiteral(_lokasi) & "';"
            ...
            totalTunjangan = tunjanganSertifikat + tunjanganHadir + tunjanganTransport + tunjanganHariLiburNasional + IIf(komponenLain2 > 0, komponenLain2, 0)
