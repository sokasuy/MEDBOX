Public Class FormMasterSuratPerintahKerja
    ...
    Public Sub New(_dbType As String, _schemaTmp As String, _schemaHRD As String, _connMain As Object, _username As String, _superuser As Boolean, _dtTableUserRights As DataTable, _entityChose As String, _addNewValues As String, _addNewFields As String, _addUpdateString As String, _lokasi As String)
        ...
        Catch ex As Exception
            Call myCShowMessage.ShowErrMsg("Pesan Error: " & ex.Message, "New FormMasterSuratPerintahKerja Error")
        End Try
    End Sub

    Private Sub FormMasterSuratPerintahKerja_FormClosed(sender As Object, e As FormClosedEventArgs) Handles Me.FormClosed
        Call myCFormManipulation.CloseMyForm(Me.Owner, Me)
    End Sub
    Private Sub FormMasterSuratPerintahKerja_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Try
            isNew = True
            Dim arrCbo() As String
            arrCbo = {"TANGGAL PENGAJUAN", "PERIODE MULAI", "PERIODE SELESAI", "PERUSAHAAN", "NO SPK", "NIP", "NAMA", "NIP KEPALA", "NAMA KEPALA"}
            ...
            tableNameHeader = CONN_.schemaHRD & ".trspkheader"
            tableNameDetail = CONN_.schemaHRD & ".trspkdetail"
            tableKey = "nospk"

            dtpAwal.Value = DateSerial(Now.Year, Now.Month, Now.Day - 7)
            dtpAkhir.Value = DateSerial(Now.Year, Now.Month, Now.Day + 7)
        Catch ex As Exception
            Call myCShowMessage.ShowErrMsg("Pesan Error: " & ex.Message, "FormMasterSuratPerintahKerja_Load Error")
        End Try
    End Sub

    Private Sub FormMasterSuratPerintahKerja_Activated(sender As Object, e As EventArgs) Handles MyBase.Activated
        Try
            Call myCDataGridViewManipulation.AutoNumberRowsForGridViewWithoutPaging(dgvDetail, dgvDetail.RowCount)
        Catch ex As Exception
            Call myCShowMessage.ShowErrMsg("Pesan Error: " & ex.Message, "FormMasterSuratPerintahKerja_Activated Error")
        End Try
    End Sub

    Private Sub FormMasterSuratPerintahKerja_KeyDown(sender As Object, e As KeyEventArgs) Handles dtpAwal.KeyDown, dtpAkhir.KeyDown, cboKriteria.KeyDown, btnCreateNew.KeyDown, tbCari.KeyDown, btnTampilkan.KeyDown, btnKeluar.KeyDown
        Try
            ...
            End If
        Catch ex As Exception
            Call myCShowMessage.ShowErrMsg("Pesan Error: " & ex.Message, "FormMasterSuratPerintahKerja_KeyDown Error")
        End Try
    End Sub
    ...
    Private Sub SetDGV(myConn As Object, myComm As Object, myReader As Object, offSet As Integer, ByRef myDataTable As DataTable, ByRef myBindingTable As BindingSource, ByRef dgvView As DataGridView, mKriteria As String, Optional gantiKriteria As Boolean = False, Optional tipe As String = "header")
        Try
            ...
            If (tipe = "header") Then
                ...
                ElseIf (mSelectedCriteria = "NIPKEPALA" Or mSelectedCriteria = "NAMAKEPALA" Or mSelectedCriteria = "NOSPK" Or mSelectedCriteria = "PERUSAHAAN") Then
                ...
                With cmbDgvEditButton
                    If Not (cekTambahButton(0)) Then
                        ...
                        .DisplayIndex = dgvView.Columns("no_spk").Index + 1
            ...
            ElseIf (tipe = "detail") Then
                stSQL = "SELECT d.rid,d." & tableKey & " as no_spk, d.kdr, d.idk, d.nip, d.nama,d.perusahaan, d.mulai, d.selesai, d.pekerjaan, d.shift,d.bagian,d.divisi,d.departemen,d.tanggalpengajuan as tanggal_pengajuan, d.created_at, d.updated_at
                        FROM " & tableNameHeader & " as h INNER JOIN " & tableNameDetail & " as d ON h." & tableKey & " = d." & tableKey & "
                        WHERE (((d." & tableKey & ")='" & myCStringManipulation.SafeSqlLiteral(mKriteria) & "'))
                        ORDER BY d.nama,d.mulai;"
        ...
        Try
            If (e.RowIndex <> -1) Then
                If (dgvHeader.RowCount > 0) Then
                    ...
                    Call SetDGV(CONN_.dbMain, CONN_.comm, CONN_.reader, 10, myDataTableDGVDetail, myBindingTableDGVDetail, dgvDetail, dgvHeader.CurrentRow.Cells("no_spk").Value, True, "detail")
                    ...
                    ElseIf (e.ColumnIndex = dgvHeader.Columns("cetak").Index) Then
                        stSQL = "SELECT h.perusahaan,h.lokasi,h.departemen as deptspk,h.periodemulai,h.periodeselesai,h." & tableKey & ",h.tanggalpengajuan,h.nipkepala,h.namakepala,h.jumlahpersonil,h.catatan as catheader,d.nip,d.nama,d.departemen,d.divisi,d.bagian,d.shift,d.mulai,d.selesai,d.pekerjaan,cast(d.mulai as time) as jammulai,cast(d.selesai as time) as jamselesai,to_char(cast(d.mulai as time),'HH24:MI') ||'-'|| to_char(cast(d.selesai as time),'HH24:MI') as jamspk,date(d.mulai) as tanggalshift, to_char(cast(d.selesai-d.mulai as time),'HH24:MI') as banyakjam
                                FROM " & tableNameHeader & " as h inner join " & tableNameDetail & " as d on h." & tableKey & "=d." & tableKey & " " & "
                                WHERE h." & tableKey & " = '" & myCStringManipulation.SafeSqlLiteral(dgvHeader.CurrentRow.Cells("no_spk").Value) & "'
    ...
    Private Sub btnCreateNew_Click(sender As Object, e As EventArgs) Handles btnCreateNew.Click
        Try
            isNew = True
            Dim fmDetailSuratPerintahKerja As New FormDetailSuratPerintahKerja(myDataTableDGVHeader, myDataTableDGVDetail, tableNameHeader, tableNameDetail, tableKey, isNew,,, Me.Name)
            Call myCFormManipulation.GoToForm(Me.MdiParent, fmDetailSuratPerintahKerja)
        Catch ex As Exception
