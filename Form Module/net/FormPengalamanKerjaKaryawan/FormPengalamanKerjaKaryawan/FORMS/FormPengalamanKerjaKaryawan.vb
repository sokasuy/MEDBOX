Public Class FormPengalamanKerjaKaryawan
    ...
    Private Sub FormPengalamanKerjaKaryawan_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Try
            ...
            tableName = "pengalamankerjakaryawan"
            isDataPrepared = True
        Catch ex As Exception
    ...
    Private Sub SetDGV(myConn As Object, myComm As Object, myReader As Object, offSet As Integer, ByRef myDataTable As DataTable, ByRef myBindingTable As BindingSource, mKriteria As String, Optional gantiKriteria As Boolean = False)
        Try
            ...
            stSQL = "SELECT rid,idk,nama,namatempatkerja as nama_tempat_kerja,posisiterakhir as posisi_terakhir,lamabekerja as lama_bekerja,tahunmasukkerja as tahun_masuk_kerja,tahunkeluarkerja as tahun_keluar_kerja,created_at,updated_at " &
                    "FROM ( " &
                        "SELECT Top " & batas & " sub.rid,sub.idk,sub.nama,sub.namatempatkerja,sub.posisiterakhir,sub.lamabekerja,sub.tahunmasukkerja,sub.tahunkeluarkerja,sub.created_at,sub.updated_at " &
                        "FROM ( " &
                            "SELECT TOP " & offSet & " tbl.rid,tbl.idk,tbl.nama,tbl.namatempatkerja,tbl.posisiterakhir,tbl.lamabekerja,tbl.tahunmasukkerja,tbl.tahunkeluarkerja,tbl.created_at,tbl.updated_at " &
        ...
        Try
            If (dgvView.RowCount > 0) Then
                ...
                If (e.ColumnIndex = dgvView.Columns("delete").Index) Then
                    ...
                    isConfirm = myCShowMessage.GetUserResponse("Apakah mau menghapus data pengalaman kerja karyawan " & dgvView.CurrentRow.Cells("nama").Value & " di " & dgvView.CurrentRow.Cells("nama_tempat_kerja").Value & "?" & ControlChars.NewLine & "Data yang sudah dihapus tidak dapat dikembalikan lagi!")
                    If (isConfirm = 6) Then
            ...
            If (cboKaryawan.SelectedIndex <> -1 And Trim(tbNamaTempatKerja.Text).Length > 0) Then
                ...
                Else
                    ...
                    If (arrDefValues(4) <> Trim(tbPosisiTerakhir.Text)) Then
                        updateString = "posisiterakhir='" & myCStringManipulation.SafeSqlLiteral(tbPosisiTerakhir.Text) & "'"
                        If (foundRows.Length > 0) Then
                    ...
                    If (arrDefValues(5) <> Trim(tbLamaBekerja.Text)) Then
                        updateString &= IIf(IsNothing(updateString), "", ",") & "lamabekerja='" & myCStringManipulation.SafeSqlLiteral(tbLamaBekerja.Text) & "'"
                        If (foundRows.Length > 0) Then
