Public Class FormSuratPeringatan
    ...
    Private Sub FormSuratPeringatan_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Try
            ...
            stSQL = "SELECT nama + ' || ' + nip as karyawan,idk,nip,nama FROM mskaryawanaktif GROUP BY nama + ' || ' + nip,idk,nip,nama ORDER BY karyawan;"
            ...
            tableName = "trsuratperingatan"
            tableNameLog = "logsuratperingatan"
            digitLength = 3
    ...
    Private Sub FormSuratPeringatan_KeyDown(sender As Object, e As System.Windows.Forms.KeyEventArgs) Handles cboKaryawan.KeyDown, dtpTanggal.KeyDown, cboKepala.KeyDown, tbKesalahan.KeyDown, rtbKeterangan.KeyDown, rtbSanksi.KeyDown, btnSimpan.KeyDown, btnKeluar.KeyDown, btnAddNew.KeyDown, tbCari.KeyDown, btnTampilkan.KeyDown, btnBackupDataLama.KeyDown
        Try
    ...
    Private Sub SetDGV(myConn As Object, myComm As Object, myReader As Object, offSet As Integer, ByRef myDataTable As DataTable, ByRef myBindingTable As BindingSource, mKriteria As String, Optional gantiKriteria As Boolean = False)
        Try
            ...
            stSQL = "SELECT rid,kdr,tanggal,idk,nip,nama,bagian,divisi,departemen,perusahaan,idkkepala as idk_kepala,nipkepala as nip_kepala,namakepala as nama_kepala,jenissp as jenis_sp,kesalahan,keterangan,sanksi,spke as sp_ke,pathtoattachment as path_to_attachment,created_at,updated_at " &
                    "FROM ( " &
                        "SELECT Top " & batas & " sub.rid,sub.kdr,sub.tanggal,sub.idk,sub.nip,sub.nama,sub.bagian,sub.divisi,sub.departemen,sub.perusahaan,sub.idkkepala,sub.nipkepala,sub.namakepala,sub.jenissp,sub.kesalahan,sub.keterangan,sub.sanksi,sub.spke,sub.pathtoattachment,sub.created_at,sub.updated_at " &
                        "FROM ( " &
                            "SELECT TOP " & offSet & " tbl.rid,tbl.kdr,tbl.tanggal,tbl.idk,tbl.nip,tbl.nama,tbl.bagian,tbl.divisi,tbl.departemen,tbl.perusahaan,tbl.idkkepala,tbl.nipkepala,tbl.namakepala,tbl.jenissp,tbl.kesalahan,tbl.keterangan,tbl.sanksi,tbl.spke,tbl.pathtoattachment,tbl.created_at,tbl.updated_at " &
            ...
            With dgvView
                ...
                .Columns("path_to_attachment").Visible = False
                ...
                .Columns("path_to_attachment").HeaderText = "ATTACHMENT"
        ...
        Try
            If (dgvView.RowCount > 0 And rbAktif.Checked) Then
                ...
                If (e.ColumnIndex = dgvView.Columns("delete").Index) Then
                    ...
                    isConfirm = myCShowMessage.GetUserResponse("Apakah mau menghapus surat peringatan karyawan " & dgvView.CurrentRow.Cells("nama").Value & " di tanggal " & dgvView.CurrentRow.Cells("tanggal").Value & "?" & ControlChars.NewLine & "Data yang sudah dihapus tidak dapat dikembalikan lagi!")
                    If (isConfirm = 6) Then
    ...
    Private Sub btnSimpan_Click(sender As Object, e As EventArgs) Handles btnSimpan.Click
        Try
            If (cboKaryawan.SelectedIndex <> -1 And cboKepala.SelectedIndex <> -1 And Trim(tbKesalahan.Text).Length > 0) Then
                ...
                If isNew Then
                    'CREATE NEW
                    'isExist = myCDBOperation.IsExistRecords(CONN_.dbMain, CONN_.comm, CONN_.reader, "rid", tableName, "nip='" & myCStringManipulation.SafeSqlLiteral(cboKaryawan.SelectedValue) & "' and tanggal='" & myCStringManipulation.SafeSqlLiteral(tbNoRekening.Text) & "'")
                    'If Not isExist Then
                    'CREATE NEW
                ...
                Else
                    ...
                    If (arrDefValues(12) <> Trim(rtbSanksi.Text)) Then
                        updateString &= IIf(IsNothing(updateString), "", ",") & "sanksi='" & myCStringManipulation.SafeSqlLiteral(rtbSanksi.Text) & "'"
                        If (foundRows.Length > 0) Then
    ...
    Private Sub rbJenisSP_CheckedChanged(sender As Object, e As EventArgs) Handles rbLisan.CheckedChanged, rbTertulis.CheckedChanged
        Try
            If (rbLisan.Checked) Then
                strJenisSP = "Lisan"
            ElseIf (rbTertulis.Checked) Then
                strJenisSP = "Tertulis"
            End If
        Catch ex As Exception
            ...
            If isNew Then
                If (isDataPrepared) Then
                    If (cboKaryawan.SelectedIndex <> -1) Then
                        ...
                        'tbPerusahaan.Text = DirectCast(cboKaryawan.SelectedItem, DataRowView).Item("perusahaan")
                        'tbDepartemen.Text = cboKaryawan.SelectedItem("departemen")
                        'tbDivisi.Text = cboKaryawan.SelectedItem("divisi")
                        'tbBagian.Text = cboKaryawan.SelectedItem("bagian")
    ...
    Private Sub lnklblAttachment_LinkClicked(sender As Object, e As LinkLabelLinkClickedEventArgs) Handles lnklblAttachment.LinkClicked
        Try
            ...
            'System.Diagnostics.Process.Start("" & lnklblAttachment.Text & "")
    ...
    Private Sub btnBackupDataLama_Click(sender As Object, e As EventArgs) Handles btnBackupDataLama.Click
        Try
            Dim isConfirm As Integer
            isConfirm = myCShowMessage.GetUserResponse("Apakah mau memproses SP yang sudah melewati periode resetnya untuk dipindah ke data history?")
            If (isConfirm = 6) Then
    ...
    Private Sub ProsesSuratPeringatanLama(sender As Object, e As EventArgs, _conn As Object, _comm As Object, _reader As Object, _type As String, Optional _nip As String = Nothing)
        Try
            ...
            If IsNothing(_nip) Then
                stSQL = "SELECT k.nip,k.hariresetsp FROM " & tableName & " as p inner join mskaryawanaktif as k ON k.nip=p.nip GROUP BY k.nip,k.hariresetsp ORDER BY k.nip;"
            Else
                stSQL = "SELECT k.nip,k.hariresetsp FROM " & tableName & " as p inner join mskaryawanaktif as k ON k.nip=p.nip WHERE k.nip='" & myCStringManipulation.SafeSqlLiteral(_nip) & "' GROUP BY k.nip,k.hariresetsp ORDER BY k.nip;"
            End If
