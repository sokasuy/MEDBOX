Public Class FormMasterRekeningKaryawan
    ...
    Private Sub FormMasterRekeningKaryawan_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Try
            ...
            stSQL = "SELECT keterangan FROM msgeneral where kategori='bank' order by keterangan;"
    ...
    Private Sub FormMasterRekeningKaryawan_KeyDown(sender As Object, e As System.Windows.Forms.KeyEventArgs) Handles cboKaryawan.KeyDown, cboBank.KeyDown, tbNoRekening.KeyDown, dtpTanggal.KeyDown, tbAtasNama.KeyDown, rtbKeterangan.KeyDown, btnSimpan.KeyDown, btnKeluar.KeyDown, btnAddNew.KeyDown, tbCari.KeyDown, btnTampilkan.KeyDown
        Try
    ...
    Private Sub SetDGV(myConn As Object, myComm As Object, myReader As Object, offSet As Integer, ByRef myDataTable As DataTable, ByRef myBindingTable As BindingSource, mKriteria As String, Optional gantiKriteria As Boolean = False)
        Try
            ...
            stSQL = "SELECT rid,idk,nama,bank,norek as no_rek,tanggal,atasnama as atas_nama,aktif,keterangan,created_at,updated_at " &
                    "FROM ( " &
                        "SELECT Top " & batas & " sub.rid,sub.idk,sub.nama,sub.bank,sub.norek,sub.tanggal,sub.atasnama,sub.aktif,sub.keterangan,sub.created_at,sub.updated_at " &
                        "FROM ( " &
                            "SELECT TOP " & offSet & " tbl.rid,tbl.idk,tbl.nama,tbl.bank,tbl.norek,tbl.tanggal,tbl.atasnama,tbl.aktif,tbl.keterangan,tbl.created_at,tbl.updated_at " &
            ...
            myDataTable = myCDBOperation.GetDataTableUsingReader(myConn, myComm, myReader, stSQL, "T_RekeningKaryawan")
        ...
        Try
            If (dgvView.RowCount > 0) Then
                ...
                If (e.ColumnIndex = dgvView.Columns("delete").Index) Then
                    ...
                    isConfirm = myCShowMessage.GetUserResponse("Apakah mau menghapus data rekening " & dgvView.CurrentRow.Cells("no_rek").Value & " di bank " & dgvView.CurrentRow.Cells("bank").Value & " milik karyawan " & dgvView.CurrentRow.Cells("nama").Value & "?" & ControlChars.NewLine & "Data yang sudah dihapus tidak dapat dikembalikan lagi!")
                    If (isConfirm = 6) Then
            ...
            If (cboKaryawan.SelectedIndex <> -1 And cboBank.SelectedIndex <> -1 And Trim(tbNoRekening.Text).Length > 0 And Trim(tbAtasNama.Text).Length > 0) Then
                ...
                Else
                    ...
                    If (arrDefValues(6) <> Trim(tbAtasNama.Text)) Then
                        updateString &= IIf(IsNothing(updateString), "", ",") & "atasnama='" & myCStringManipulation.SafeSqlLiteral(tbAtasNama.Text) & "'"
                        If (foundRows.Length > 0) Then
