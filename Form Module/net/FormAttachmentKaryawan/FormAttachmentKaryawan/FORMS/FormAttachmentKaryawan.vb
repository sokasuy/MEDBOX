Public Class FormAttachmentKaryawan
    ...
    Public Sub New(_dbType As String, _ConnMain As Object, _username As String, _superuser As Boolean, _dtTableUserRights As DataTable, _addNewValue As String, _addNewFields As String, _addUpdateString As String, _groupData As String, _idk As String, _nama As String, Optional _nip As String = "-")
        Try
    ...
    Private Sub FormAttachmentKaryawan_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Try
            ...
            tableName = "attachmentkaryawan"

            isDataPrepared = True

            If (karyawan.nip <> "-") Then
                strTujuanFile = "G:\ATTACHMENT\KARYAWAN\" & karyawan.idk & "\" & karyawan.nip & "\"
            Else
                strTujuanFile = "G:\ATTACHMENT\KARYAWAN\" & karyawan.idk & "\"
            End If
    ...
    Private Sub SetDGV(myConn As Object, myComm As Object, myReader As Object, offSet As Integer, ByRef myDataTable As DataTable, ByRef myBindingTable As BindingSource, mKriteria As String, Optional gantiKriteria As Boolean = False)
        Try
            ...
            stSQL = "SELECT rid,idk,nip,nama,keterangan,pathtofile as path_to_file,namafile as nama_file,groupdata as group_data,ispublic as is_public,created_at,updated_at " &
                    "FROM ( " &
                        "SELECT Top " & batas & " sub.rid,sub.idk,sub.nip,sub.nama,sub.groupdata,sub.ispublic,sub.keterangan,sub.pathtofile,sub.namafile,sub.created_at,sub.updated_at " &
                        "FROM ( " &
                            "SELECT TOP " & offSet & " tbl.rid,tbl.idk,tbl.nip,tbl.nama,tbl.groupdata,tbl.ispublic,tbl.keterangan,tbl.pathtofile,tbl.namafile,tbl.created_at,tbl.updated_at " &
        ...
        Try
            If (dgvView.RowCount > 0) Then
                ...
                If (e.ColumnIndex = dgvView.Columns("delete").Index) Then
                    ...
                    isConfirm = myCShowMessage.GetUserResponse("Apakah mau menghapus file " & dgvView.CurrentRow.Cells("nama_file").Value & " dari karyawan " & dgvView.CurrentRow.Cells("nama").Value & " pada group data " & dgvView.CurrentRow.Cells("group_data").Value & "?" & ControlChars.NewLine & "Data yang sudah dihapus tidak dapat dikembalikan lagi!")
                    If (isConfirm = 6) Then
    ...
    Private Sub lblEntryType_Click(sender As Object, e As EventArgs) Handles lblEntryType.Click
